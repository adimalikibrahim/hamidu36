                        ...
                        <div class="card border rounded-3 p-2 p-sm-4">
                            <div class="card-header p-0">
                                <div class="d-flex justify-content-between align-items-center p-3 bg-light rounded-2">
                                    <div>
                                        <h4 class="mb-0">{{ $angk->angkatan }}</h4>
                                    </div>
                                    <div>
                                        <h4 class="text-success mb-0 plan-price">
                                            <!--@foreach($angAng->where('id_angkatan', $angk->id_angkatan))-->
                                                
                                            <!--@endforeach-->
                                            {{ $alumni->where('id_angkatan', $angk->id_angkatan)->count() }}
                                            </h4>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card-footer text-center d-grid pb-0 m-0 p-0">
                                <a href="/hamidu/anggota-angkatan/{{ $angk->id_angkatan }}" class="btn btn-dark mb-0">Anggota {{$angk->nama_angkatan}}</a>
                            </div>
                        </div>
                    </div>
                @endforeach
